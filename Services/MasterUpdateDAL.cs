...
namespace TMS_Api.Services
{
    public class MasterUpdateDAL
    {
        ...
        public async Task<ResponseMessage> SaveTransporterType(TransporterTypeDto info)
        {
            ...
            try
            {
                TransporterType data = await _context.TransporterType.FromSqlRaw("SELECT Top 1 * FROM TransporterType WHERE REPLACE(TypeName,'','')=REPLACE(@name,'','')", new SqlParameter("@name", info.TypeName)).SingleOrDefaultAsync();
                if (data != null)
                {
                    msg.Status = false;
                    msg.MessageContent = "Name duplicate!";
        ...
        public async Task<ResponseMessage> UpdateTransporterType(TransporterTypeDto info)
        {
            ...
            try
            {
                TransporterType type = await _context.TransporterType.FromSqlRaw("SELECT * FROM TransporterType WHERE REPLACE(TypeName,' ','')=REPLACE(@name,' ','') ", new SqlParameter("@name", info.TypeName)).SingleOrDefaultAsync();
                if (type == null)
                {
                    msg.Status = false;
                    msg.MessageContent = "Data Not Found";
        ...
        public async Task<ResponseMessage> DeleteTransporterType(string id)
        {
            ...
            try
            {
                ...
                {
                    Transporter transporter = await _context.Transporter.FromSqlRaw("SELECT * FROM Transporter WHERE  REPLACE(TransporterType,' ','')=REPLACE(@id,' ','')", new SqlParameter("@id", type.TypeName)).SingleOrDefaultAsync();
                    if (transporter == null)
                    {
        ...
        public async Task<ResponseMessage> SaveTransporter([FromForm] TransporterDto info)
        {
            ...
            try
            {
                Transporter transporter = await _context.Transporter.FromSqlRaw("SELECT Top 1 * FROM Transporter WHERE REPLACE(TransporterCode,' ','')=REPLACE(@name,' ','')", new SqlParameter("@name", info.TransporterCode)).SingleOrDefaultAsync();
                if (transporter != null)
                {
                    msg.Status = false;
                    msg.MessageContent = "Data Not Found";
        ...
        public async Task<ResponseMessage> UpdateTransporter([FromForm] TransporterDto info)
        {
            ...
            try
            {
                Transporter transporter= await _context.Transporter.SingleOrDefaultAsync(t => t.TransporterCode == info.TransporterCode);
                ...
                {
                    transporter.TransporterCode = info.TransporterCode;
                    ...
                    transporter.TransporterType = info.TransporterType;
        ...
        public async Task<ResponseMessage> SaveTrailer([FromForm] TrailerDto info)
        {
            ...
            try
            {
                ...
                //Trailer trailer = await _context.Trailer.FromSqlRaw("SELECT Top 1 * FROM Trailer Order By VehicleRegNo").SingleOrDefaultAsync();
                if (trailer != null)
                {
                    msg.Status = false;
                    msg.MessageContent = "Data Not Found";
        ...
        public async Task<ResponseMessage> UpdateDriver([FromForm] DriverDto info)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    driver.Transporter = info.Transporter;
